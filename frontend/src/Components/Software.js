...
function Software() {
  ...
  const addSoftwareItemTofavouritelist = (i, image, name, url) => {
  ...
  /* Creating an asynchronous function to perform the API call using axios. I am requesting the data
     ...
     if not, an alert will come up asking the user to enter an artists name before trying to search for software.  */
  async function searchForSongs() {
    const apiUrl = `/software/${artist}`;
    try {
      if (artist) {
              ...
              return (
                <div className="individualElementContainer">
                  ...
                  <button
                    ...
                    onClick={() =>
                      addSoftwareItemTofavouritelist(
                        i,
                        res.artworkUrl100,
                        res.artistName,
                        res.artistViewUrl
